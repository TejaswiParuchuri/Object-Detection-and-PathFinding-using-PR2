...
class RobotActionsServer:
    ...
    def __init__(self, object_dict, root_path, random_seed=10, simulation=False):
    ...
    def execute_action(self, req):
        ...
        calling_params.append(self.simulation)
    ...
    def execute_sense(self, obj_name, current_state, simulation=False):
        current_state = json.loads(current_state)
        next_state = copy.deepcopy(current_state)

        if simulation:
            action_str="sense|{}".format(obj_name)
            print("[action_server]: Sensing {}".format(action_str))
            self.action_publisher.publish(String(data=action_str))
            result = rospy.wait_for_message("/sense_report",String)
            print("Result: {}".format(result))
            if("prediction|0.0" in str(result.data)):
                next_state["sees_can"] = True
            else:
                next_state["sees_can"] = False
        else:
            if "can" in obj_name:
                next_state["sees_can"] = True
            else:
