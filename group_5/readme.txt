...
Known Issues:
	...
	-> Running the sense_demo.py, the second time will use the image from the previous robot state. Hence, it will try to predict
	   based off the sensed image at the previous state. There is a problem with the way callback methods work. It is somehow not able
	   to update the data (Image) on every subsequent runs. Hence, it chooses the image from previous timesteps.
